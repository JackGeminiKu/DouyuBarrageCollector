...
namespace Douyu.Client
{
    public static class DbService
    {
        ...
        {
            var command = string.Format("insert into chat_message" +
            ...
            if (ExecuteNonQuery(command) != 1)
                LogService.InfoFormat("保存弹幕失败: 返回值不为1! ({0})", command);
        }

        public static void SaveGiftMessage(GiftMessage giftMessage)
        {
            var command = string.Format("insert into gift_message" +
            ...
            if (ExecuteNonQuery(command) != 1)
                LogService.InfoFormat("保存礼物失败: 返回值不为1! ({0})", command);
        }

        public static void SaveChouqinMessage(ChouqinMessage message)
        {
            var command = string.Format("insert into chouqin_message" +
            ...
            if (ExecuteNonQuery(command) != 1)
                LogService.InfoFormat("保存酬勤失败: 返回值不为1! ({0})", command);
        }
    ...
    public class DbException : Exception
    {
        ...
        public DbException(string format, params object[] args) : this(string.Format(format, args)) { }
    }
}
